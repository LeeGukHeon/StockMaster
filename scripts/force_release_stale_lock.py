...
from app.ops.maintenance import force_release_stale_lock
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date, run_standalone_job


def main() -> int:
    parser = argparse.ArgumentParser(description="Force release stale active locks.")
    ...
    log_and_print(f"Stale locks released. run_id={result.run_id} count={result.row_count}")
    return 0
