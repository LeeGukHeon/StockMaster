...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate alpha model v1.")
...
def main() -> int:
    ...
    result = validate_alpha_model_v1(
        settings,
        as_of_date=args.as_of_date,
        horizons=args.horizons,
    )
    logger.info(
        "Alpha model validation completed.",
        extra={"run_id_value": result.run_id, "row_count": result.row_count},
    )
    print(
        f"Alpha model validation completed. as_of_date={args.as_of_date.isoformat()} "
