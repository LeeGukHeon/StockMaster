...
from app.release.reporting import render_daily_research_report
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date, run_standalone_job


def main() -> int:
    parser = argparse.ArgumentParser(description="Render the daily research report.")
    ...
    log_and_print(
        f"Daily research report rendered. run_id={result.run_id} artifacts={len(result.artifact_paths)}"
    )
    return 0
