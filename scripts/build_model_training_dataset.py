...
from app.ml.dataset import build_model_training_dataset
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build alpha model training dataset.")
...
def main() -> int:
    ...
    result = build_model_training_dataset(
        settings,
    ...
    logger.info(
        "Model training dataset completed.",
        extra={"run_id_value": result.run_id, "row_count": result.row_count},
    )
    print(
        f"Model training dataset completed. train_end_date={args.train_end_date.isoformat()} "
