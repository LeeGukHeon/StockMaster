...
from app.ops.maintenance import cleanup_disk_watermark
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date, run_standalone_job


def main() -> int:
    parser = argparse.ArgumentParser(description="Apply disk watermark cleanup policy.")
    ...
    log_and_print(
        f"Disk watermark cleanup completed. run_id={result.run_id} status={result.status}"
    )
    return 0
