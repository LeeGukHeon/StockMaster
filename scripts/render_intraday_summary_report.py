...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render the intraday summary report.")
    parser.add_argument("--session-date", required=True, type=parse_date)
    ...
    result = run_standalone_job(
        ...
        as_of_date=args.session_date,
        dry_run=args.dry_run,
        policy_config_path=None,
        runner=_runner,
    )
    log_and_print(
        f"Intraday summary report rendered. run_id={result.run_id} artifacts={len(result.artifact_paths)}"
    )
    return 0
