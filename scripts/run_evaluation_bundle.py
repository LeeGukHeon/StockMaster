...
from app.ops.bundles import run_evaluation_bundle
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the evaluation scheduler bundle.")
    ...
    log_and_print(f"Evaluation bundle completed. run_id={result.run_id} status={result.status}")
    return 0
