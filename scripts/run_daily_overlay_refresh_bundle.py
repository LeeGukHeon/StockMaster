...
from app.ops.bundles import run_daily_overlay_refresh_bundle
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the daily overlay refresh bundle.")
    ...
    log_and_print(
        f"Daily overlay refresh bundle completed. run_id={result.run_id} status={result.status}"
    )
    return 0
