...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build calibration diagnostic rows from frozen outcomes."
    )
    ...
    parser.add_argument("--bin-count", type=int, required=True)
...
def main() -> int:
    ...
    print(f"Calibration diagnostics completed. run_id={result.run_id} rows={result.row_count}")
    return 0
