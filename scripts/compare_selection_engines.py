...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare selection engines.")
...
def main() -> int:
    ...
    result = compare_selection_engines(
        settings,
    ...
    logger.info(
        "Selection engine comparison completed.",
        extra={"run_id_value": result.run_id, "row_count": result.row_count},
    )
    print(
        f"Selection engine comparison completed. range={args.start_selection_date.isoformat()}.."
        f"{args.end_selection_date.isoformat()} run_id={result.run_id} rows={result.row_count}"
    )
    return 0
