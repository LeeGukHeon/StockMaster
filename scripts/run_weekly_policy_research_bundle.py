...
from app.ops.bundles import run_weekly_policy_research_bundle
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the weekly policy research bundle.")
    ...
    log_and_print(
        f"Weekly policy research bundle completed. run_id={result.run_id} status={result.status}"
    )
    return 0
