...
from app.ops.health import materialize_health_snapshots
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date, run_standalone_job


def main() -> int:
    parser = argparse.ArgumentParser(description="Materialize ops health snapshots.")
    ...
    log_and_print(
        f"Health snapshots materialized. run_id={result.run_id} rows={result.row_count}"
    )
    return 0
