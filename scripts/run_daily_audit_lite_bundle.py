...
from app.ops.bundles import run_daily_audit_lite_bundle
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the daily audit-lite bundle.")
    ...
    log_and_print(
        f"Daily audit-lite bundle completed. run_id={result.run_id} status={result.status}"
    )
    return 0
