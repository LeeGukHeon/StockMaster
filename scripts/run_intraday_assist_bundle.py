...
from app.ops.bundles import run_intraday_assist_bundle
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the intraday assist scheduler bundle.")
    ...
    log_and_print(
        "Intraday assist bundle completed. "
        f"run_id={result.run_id} status={result.status} checkpoint={args.checkpoint_time or 'auto'}"
    )
    return 0
