...
from app.ops.maintenance import recover_incomplete_runs
from scripts._ops_cli import load_cli_settings, log_and_print, parse_date, run_standalone_job


def main() -> int:
    parser = argparse.ArgumentParser(description="Recover incomplete or failed ops runs.")
    ...
    log_and_print(f"Recovery run completed. run_id={result.run_id} recovered={result.row_count}")
    return 0
