...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render the release candidate checklist report.")
    ...
    log_and_print(
        f"Release candidate checklist rendered. run_id={result.run_id} artifacts={len(result.artifact_paths)}"
    )
    return 0
