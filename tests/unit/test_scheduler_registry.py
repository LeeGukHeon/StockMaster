...
def test_scheduler_registry_contains_expected_jobs() -> None:
    ...
    assert SCHEDULED_JOB_MAP["daily_close"].run_times == ("17:30",)
    ...
    assert SCHEDULED_JOB_MAP["daily_overlay_refresh"].run_times == ("21:30",)
    assert SCHEDULED_JOB_MAP["daily_overlay_refresh"].date_semantics == "trading_day"
    assert SCHEDULED_JOB_MAP["docker_build_cache_cleanup"].run_times == ("04:30",)
    ...
    assert SCHEDULED_JOB_MAP["weekly_calibration"].run_times == ("10:00",)
    ...
    assert SCHEDULED_JOB_MAP["weekly_policy_research"].run_times == ("14:00",)
