    ...
    def fetch_daily_ohlcv(
        ...
        *,
        ...
        return FakeDailyProbe(
            ...
            payload={"output2": [row], "symbol": symbol},
            raw_json_path="raw.json",
            raw_parquet_path="raw.parquet",
        )


def test_sync_daily_ohlcv_populates_fact_table(tmp_path):
    ...
    with duckdb_connection(settings.paths.duckdb_path) as connection:
        connection.execute(
            ...
            VALUES
                ('005930', '삼성전자', 'KOSPI', TRUE, 'test', now()),
                ('000660', 'SK하이닉스', 'KOSPI', TRUE, 'test', now())
            """
        )
        connection.execute(
            """
